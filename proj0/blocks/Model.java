...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        if (piece.height() + row > _height || piece.width() + col > _width) {
            return false;
        } else {
            for (int i = row; i < row + piece.height(); i++) {
                for (int j = col; j < col + piece.width(); j++) {
                    if (_cells[i][j]
                            && piece.getPositions()[i - row][j - col]) {
                        return false;
                    }
                }
            }
    ...
    void place(Piece piece, int row, int col) {
        ...
        for (int i = row; i < row + piece.height(); i++) {
            for (int j = col; j < col + piece.width(); j++) {
                if (piece.getPositions()[i - row][j - col]) {
                    _cells[i][j] = piece.getPositions()[i - row][j - col];
                    this._score += 1;
                }
            }
        }
    ...
    void clearFilledLines() {
        ...
        boolean[] rowsToClear = new boolean[this._height];
        boolean[] colsToClear = new boolean[this._width];
        for (int i = 0; i < this._height; i++) {
            rowsToClear[i] = false;
        }
        for (int j = 0; j < this._width; j++) {
        ...
        nrows = ncols = 0;
        for (int i = 0; i < this._height; i++) {
            if (counts[0][i] == this._width) {
                nrows += 1;
                rowsToClear[i] = true;
            }
        }
        for (int j = 0; j < this._width; j++) {
            if (counts[1][j] == this._height) {
                ncols += 1;
                colsToClear[j] = true;
            }
        }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        if (nrows == 0 && ncols == 0) {
            ...
            int clearPointsNum;
            if (nrows == 0 || ncols == 0) {
                clearPointsNum = nrows * this._width
                        + ncols * this._height;
            } else {
                clearPointsNum  = nrows * this._width
                        + ncols * this._height - (nrows + ncols - 1);
            }
            int bonus = (nrows * this._width + ncols * this._height)
                    * this._streakLength;
            return bonus + clearPointsNum;
        }
    }
    ...
    void deal(Piece piece) {
        if (piece == null) {
            throw badArgs("piece is null,can not add to hand");
        } else {
