    ...
    public void announceWin(PieceColor player) {
        ...
        if (_board.getWinner() != null) {
            if (_board.getWinner() == EMPTY) {
                collector = showOptions("Draw","Outcome",
                        "plain","new", options);
            } else {
                collector = showOptions(String.format(
                        "%s wins.", _board.getWinner()),"Outcome",
                        "plain","new", options);
            }
        } else {
            collector = showOptions("Call announceWin when winner == null",
                    "Outcome", "plain","new", options);
        }
        if (collector == 0) {
            setAIMode("red", false);
            setAIMode("blue", true);
