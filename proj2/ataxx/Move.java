...
class Move {
    ...
    Move(int col0, int row0, int col1, int row1) {
        ...
        _isExtend = (abs(col0 - col1) <= 1) && (abs(row0 - row1) <= 1)
                && (col0 != col1 || row0 != row1);
        _isJump = (!isExtend())
                && (abs(col0 - col1) <= 2) && (abs(row0 - row1) <= 2);
    }
    ...
    public String toString() {
        if (this.isPass()) {
            return "-";
        } else if (this.isExtend() || this.isJump()) {
            return String.format("%c%d-%c%d", _col0,
                    _row0 - '0', _col1, _row1 - '0');
        }
    ...
    /** Get method for AllMove.
     * @return ALLMoves in a 4D-Matrix.
     */
